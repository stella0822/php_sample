...
//echo "<script>alert('Authenticate.php')</script>";
if(isset($_POST["vcode"])){
...

	{
		...
		$lockTime = date('Y-m-d H:i:s',time() - 60 * 60);

		$query="SELECT COUNT(*),UNIX_TIMESTAMP(locktime) AS locktimestamp FROM vtiger_user_loginlocked WHERE NAME = '".$_SESSION['login_user_name']."' AND ip = '".$_SERVER['REMOTE_ADDR']."'  AND locktime > '".$lockTime."'";

		$checkRow = $adb->query($query);

		if(is_array($checkRow)) {foreach($checkRow as $row){$templocktime=$row['locktimestamp']; }}

		if($templocktime){

			$validTime = date('Y-m-d H:i:s',((int)$templocktime + 60 * 60));

			echo "<script>alert('Lock the account until ".$validTime."。')</script>";
			...
			if($templocknum>=10){
				echo "<script>alert('You have been locked more than 10 times. Please contact the administrator.')</script>";

				require_once 'Login.php';
			}
			else{

				$query="delete from vtiger_user_loginlocked where name = '".$_SESSION['login_user_name']."' and ip = '".$_SERVER['REMOTE_ADDR']."'";
				...
				if($focus->is_authenticated())
				{
					...
					session_unregister('login_code_error');
...

	{
		...
		$_SESSION['login_code_error']="Wrong verify code.";
		header("Location: index.php");
